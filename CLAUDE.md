...
With **External Secrets**, each odoo.conf credential picks its own Vault key via `externalsecrets.properties.odoo.<cred>.{key,property}` (helper `..externalsecrets.odooConfData`, shared by the normal + hook ExternalSecrets). The DB password (`postgresqlPassword` → odoo.conf `db_password`) defaults its `key` to `externalsecrets.postgresqlKey` — the same key the bundled-PostgreSQL secret reads — so the DB password lives in Vault **once** and is shared rather than duplicated under `odooKey` (falls back to `odooKey` when `postgresqlKey` is unset, e.g. an external DB). The Odoo master password (`adminPasswd` → `admin_passwd`) defaults its `key` to `odooKey`. Any `key` can be overridden to point a credential at a different Vault key.
